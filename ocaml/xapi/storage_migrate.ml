...
let update_snapshot_info ~dbg ~sr ~vdi ~url ~dest ~dest_vdi ~snapshot_pairs =
	...
	in
	(* Check the leaf VDIs have the same content_id. *)
	let local_vdi_info = find_vdi ~vdi ~vdi_info_list:local_vdis in
	let remote_vdi_info = find_vdi ~vdi:dest_vdi ~vdi_info_list:remote_vdis in
	assert_content_ids_match local_vdi_info remote_vdi_info;
	 ...
	 * - Set is_a_snapshot = true for the remote vdi *)
	List.iter

...
type configuration_change = {
  old_vms_leaving: (API.ref_host * VMResources.t) list;             (** existing VMs which are leaving *)
  old_vms_arriving: (API.ref_host * VMResources.t) list;            (** existing VMs which are arriving *)
  hosts_to_disable: API.ref_host list;                              (** hosts to pretend to disable *)
  num_failures: int option;                                         (** new number of failures to consider *)
  new_vms_to_protect: VMResources.t list;                           (** new VMs to restart *)
}
...
let string_of_configuration_change ~__context (x: configuration_change) = 
  ...
  Printf.sprintf "configuration_change = { old_vms_leaving = [ %s ]; new_vms_arriving = [ %s ]; hosts_to_disable = [ %s ]; num_failures = %s; new_vms = [ %s ] }"
    (String.concat "; " (List.map (fun (h, vm_res) -> Printf.sprintf "%s %s" (string_of_host h) (VMResources.to_string vm_res)) x.old_vms_leaving))
    (String.concat "; " (List.map (fun (h, vm_res) -> Printf.sprintf "%s %s" (string_of_host h) (VMResources.to_string vm_res)) x.old_vms_arriving))
    (String.concat "; " (List.map string_of_host x.hosts_to_disable))
    (Opt.default "no change" (Opt.map string_of_int x.num_failures))
    (String.concat "; " (List.map VMResources.to_string x.new_vms_to_protect))
...
let host_of_non_agile_vm ~__context all_hosts_and_snapshots_sorted vm_res =
	let open VMResources in
	match vm_res with
	| {status = `Local vm_ref; vm_rec = snapshot} -> begin
...
let compute_restart_plan ~__context ~all_protected_vms ~live_set ?(change=no_configuration_change) num_failures =
	...
	let vms_to_ensure_running =
		List.map
			(fun (vm_ref, vm_rec) ->
				VMResources.of_ref_and_record ~__context vm_ref vm_rec)
			all_protected_vms
	in
	...
	let vms_to_ensure_running = List.map (fun vm ->
		...
		match is_leaving, is_arriving with
			| _, true ->
				...
				update_record vm {vm.vm_rec with API.vM_resident_on = host}
			| true, false ->
				update_record vm {vm.vm_rec with API.vM_resident_on = Ref.null}
			| _, _ -> vm)
	...
	(* Any deterministic ordering is fine here: *)
	let vms_to_ensure_running = List.sort VMResources.compare vms_to_ensure_running in
	...
	let vm_accounted_to_host = function
		| {VMResources.status = `Local vm_ref; vm_rec} ->
		...
		| {VMResources.status = `Incoming host_ref} -> Some host_ref
	in
	...
	let hosts_and_memory = List.map (fun host ->
		...
		let arriving_memory = sum (List.map (fun (_, vm) ->
			let snapshot = vm.VMResources.vm_rec in
	...
	let vms_and_memory =
		List.map
			(fun vm -> vm, total_memory_of_vm ~__context Memory_check.Static_max (vm.VMResources.vm_rec))
	...
	let agile_vms_and_memory = List.map (fun vm -> vm, VMResources.assoc vm vms_and_memory) agile_vms in

...
let host_of_non_agile_vm ~__context all_hosts_and_snapshots_sorted vm =
	...
	| HA_VM.In_db (vm_ref, snapshot) -> begin
		match (List.filter (fun (host, _) ->
			try Xapi_vm_helpers.assert_can_boot_here_no_memcheck ~__context ~self:vm_ref ~host ~snapshot; true
...
let compute_restart_plan ~__context ~all_protected_vms ?(change=no_configuration_change) num_failures = 
	...
	let assert_agile = function
		| HA_VM.In_db (vm_ref, _) -> (try Helpers.vm_assert_agile ~__context ~self:vm_ref; true with _ -> false)
		| HA_VM.Not_in_db (_, srs, networks, _) ->
			List.for_all (fun sr -> Helpers.is_sr_properly_shared ~__context ~self:sr) srs &&
			List.for_all (fun network -> Helpers.is_network_properly_shared ~__context ~self:network) networks
	in

	let agile_vms, not_agile_vms = List.partition (fun vm -> assert_agile vm) vms_to_ensure_running in
